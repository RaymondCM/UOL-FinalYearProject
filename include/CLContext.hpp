...
public:
	...
	void InitialiseArguments(int argc, char** argv) {
		for (int i = 1; i < argc; i++) {
			...
			else if (strcmp(argv[i], "-l") == 0) { ListPlatforms(); }
			else if (strcmp(argv[i], "-h") == 0) { PrintArgumentsHelp(); }
		}
	}
	...
	void AddSources(cl::Program::Sources& sources, const std::string& filePath) {
		...
		if (!file.good())
		{
			std::exception e(("Kernel File (" + filePath + ") does not exist/you do not have access.").c_str());
